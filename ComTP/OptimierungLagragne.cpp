...
PetscErrorCode main(int argc,char **argv)
{
  ...
  ierr = TaoSetType(tao,TAONLS);CHKERRQ(ierr);
...
double Lagrange(Vec PHI, double dt, int L, double Anfang){
	
	ierr = VecGetArray(PHI,&phi);CHKERRQ(ierr);
	for(int i = 0;i<L;i++){
		if(i==0){
			summe += pow(((phi[i+1]-Anfang)/(2*dt)),2)/2+cos(phi[i]);
		}else{
			summe += pow(((phi[i+1]-phi[i])/(2*dt)),2)/2+cos(phi[i]);
		}
	}
	return summe*dt;
	ierr = VecRestoreArray(PHI,&phi);CHKERRQ(ierr);
}
